...
const linkVariants = cva(
	'inline-flex items-center outline-none transition-colors',
	{
		variants: {
			variant: {
				block: 'h-8 rounded-full bg-foreground/10 p-4 text-sm hover:bg-foreground/[0.15]',
			},
		},
	}
);

type LinkProps = Omit<ComponentProps<typeof IntlLink>, 'href'> & VariantProps<typeof linkVariants> & {
	href?: string;
};
